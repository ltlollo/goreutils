...
setup_file(void) {
	...
	unlikely_if((file_beg = mmap(NULL, file_len, PROT_READ | PROT_WRITE,
	                             MAP_PRIVATE | MAP_POPULATE, file_fd,
	                             0)) == MAP_FAILED) {
		err(1, "mmap");
	}
	file_curs = file_beg;
...
setup_diff(void) {
	...
	unlikely_if((diff_fd = open(diff_name, O_RDWR | O_CREAT, 0644)) ==
	            -1) {
		err(1, "open");
	}
	...
	unlikely_if((diff_beg = mmap(NULL, diff_len, PROT_READ | PROT_WRITE,
	                             MAP_SHARED, diff_fd, 0)) == MAP_FAILED) {
		err(1, "mmap");
	}
	diff_end = diff_beg + diff_len;
	unlikely_if(dirty == true) {
...
set_col_table(unsigned char *slice) {
	...
	for (; i < nlines && slice < file_end; ++i, slice += 16) {
		for (int j = 0; j < 16; ++j) {
			...
			for (size_t k = j * 6 + i * 96;
			     k < j * 6 + i * 96 + 6ull; ++k) {
				set_vec3(&col[k], rc / 3.f, gc / 31.f,
				         bc / 2.f);
			}
		}
	}
...
set_vrx_table(void) {
	for (int j = 0; j < nlines; ++j) {
		for (int i = 0; i < 16; ++i) {
			size_t k = i * 6 + j * 96;
			set_vec2(&vrx[0 + k], nx((ix + i + 0) / nch),
			         ny(iy * (j + 1)));
			set_vec2(&vrx[1 + k], nx((ix + i + 1) / nch),
			         ny(iy * (j + 1)));
			set_vec2(&vrx[2 + k], nx((ix + i + 1) / nch),
			         ny(iy * (j + 2)));
			set_vec2(&vrx[3 + k], nx((ix + i + 1) / nch),
			         ny(iy * (j + 2)));
			set_vec2(&vrx[4 + k], nx((ix + i + 0) / nch),
			         ny(iy * (j + 2)));
			set_vec2(&vrx[5 + k], nx((ix + i + 0) / nch),
			         ny(iy * (j + 1)));
		}
	}
}
...
static void attr(unused) check_shader(GLuint shader) {
	GLint len, res;
	char *log;
	...
	while (true) {
		...
		case 'r':
			...
			if (c == 'j') {
				...
				istream->code = go;
				istream->imm = clampll(istream->imm, 0,
				                       file_end - file_beg);
			} else if (c == 'r') {
				istream->code = rep;
				unlikely_if(istream->imm == 0) {
...
exec_ilist(unsigned char *curr, instr *ibeg, instr *iend) {
	while (likely(ibeg != iend)) {
		...
		case rep:
			...
			return curr;
		case go:
			debug_assert(ibeg->imm >= 0 &&
			             ibeg->imm <= file_end - file_beg);
...
map_file_shr(bool shr) {
	...
	unlikely_if((file_beg = mmap(NULL, file_len, PROT_WRITE | PROT_READ,
	                             flag, file_fd, 0)) == MAP_FAILED) {
		err(1, "mmap");
	}
	file_end = file_beg + file_len;
}
...
commit_changes_dirty(void) {
	...
	unsigned long long size = nbyte(arr->size);
	while (likely(size != 0 && (unsigned char *)off < diff_end)) {
		unlikely_if(file_beg + *off + size > file_end ||
...
should_merge(void) {
	...
	ans res = y;
	do {
		perr("backup with unstashed changes present, merge/display "
		     "them?"
